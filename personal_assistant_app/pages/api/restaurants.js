...
// מחזיר רשימת מסעדות מתוך OpenStreetMap (Nominatim + Overpass), ללא API key.
// עדכני אימייל אמיתי ב-User-Agent בהתאם לכללי Nominatim.
...
export default async function handler(req, res) {
  try {
    ...
    const geoRes = await fetch(geoUrl, {
      headers: {
        "User-Agent":
          "horizon-personal-assistant/1.0 (contact: your-email@example.com)",
      },
    });
    if (!geoRes.ok) {
      ...
      return res
        ...
        .json({ error: "nominatim_error", details: t.slice(0, 300) });
    }
    ...
    // 2) שאילתת Overpass למסעדות סביב הנ"צ
...
out center tags;`;
    ...
    if (!ovRes.ok) {
      ...
      return res
        .status(ovRes.status)
        .json({ error: "overpass_error", details: t.slice(0, 500) });
    }
    ...
    return res
      .status(200)
      .json({ query: { location, cuisine }, restaurants: items });
  } catch (e) {
